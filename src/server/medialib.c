...
int mlib_getchnlist(struct mlib_listentry_st **result,int *resnum)      // 创建频道数组
{
    ...
    char path[PATHSIZE]; //存放媒体库的位置，/var/media/
    glob_t globres;
    snprintf(path,PATHSIZE,"%s/*",server_conf.media_dir);//解析媒体文件目录放入path中
    // 1.路径 2.特殊要求 3.出错路径不关心 4.解析结果
    // globres里面解析出来的就是路径加个数
    if(glob(path,0,NULL,&globres)) // 将path中的所有路径全部解析出来存放到globres中
    {
        // 错误
    ...
    for (int i = 0; i < globres.gl_pathc; i++)
    {
        ...
        if(channelptr != NULL)
        {
            ...
            memcpy(channel+channelptr->chnid,channelptr,sizeof(*channelptr)); //把每一个频道的MP3的路径存放到channel中
    ...
    *result = realloc(mlibptr,sizeof(struct mlib_listentry_st) * num);//调整mlibptr的大小，若size比之前的小，那么就直接返回mlibptr，如果size比之前大，那么就重新分配内存
    if(*result == NULL)
    {
...
int mlib_readchnl(chnid_t chnid,void* buf,size_t size)     // 读取每一个频道的信息，具体是音频信息
{
    // 仿照read原语
    // 这个环节进行了流量控制
    int len = 0;
...
struct channel_context_st *pathtoentry(const char *path)
{
    ...
    me->desc = strdup(linebuf); //strdup会在堆中分配一块新的内存，用于存储linebuf的副本
